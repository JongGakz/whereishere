...
public class BasicController {
    ...
    public void initMemberAndCompany() {
        ...
        eventPostService.save(new EventPost(companyServiceImpl.findCompanyByCompanyName("별다방"), "아침이벤트", "아침에는 아메리카노가 이천원 입니다.", "이미지6.png", LocalDate.of(2023, 5, 8)));
        ...
        reviewPostRepository.save(new ReviewPost(member, company, "지평선시네마", member.getName(), "작은 영화관 대한민국 1호점", " 좌석은 65석, 34석 뿐이지만 집중도 잘되고 조명, 사운드, 스피커 등 시스템도 뒤떨어지지 않아요. 대형 영화관에 작은 상영관보다 이곳이 훨씬 좋아요. 내부는 예전에 가본 서울극장과 비슷한 느낌이에요. 그리고 먹거리도 저렴한데 맛도 있어요. 올망졸망 작은 공간이지만 알차게 되어있어요. 그리고 큰 상영관과 동시상영할 정도로 개봉속도로 빨라요.", "지평선시네마.png", "지평선시네마2.png",6,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member3, company, "울산성모안과의원", member3.getName(), "알레르기로 병원다녀왔어요ㅠㅠ", "예전에 리식 수술하러 간 적 있었는데 이번에는 안구건조증이랑 알러지 치료하러 다녀왔어요! 선생님이 친절하고 꼼꼼하게 진료를 잘해주셔서 이제 다른 곳에 못가겠어요. 근데 갈 때마다 대기시간이 길어서 넉넉히 시간 잡고 가야됩니다.... 그래도 진료를 잘해주시니 그 정도는 기다릴 수 있어요! ", "울산성모안과 1", "울산성모안과 2",3,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member2, company, "돈삼이네",member2.getName(),"암사동 삼겹살 생고기 맛집 돈삼이네 암사역과 굽은다리역 중간쯤에 위치한 돈삼이네.", "돌판에 고기, 김치 부추를 깔아 줘서 같이 구워먹으면 진짜 꿀맛! 기본반찬도 맛있고 가격도 착해서 맛있어요 마지막은 볶음밥 국룰이죠! 근데 여기에 치즈 볶음밥까지 먹는다면? 더이상 말이 필요없죠!! 위에 김가루까지 덮어준다? 말해뭐해... 암사동에서 생고기 삼겹살을 먹고 싶다면 돈삼이네로!", "돈삼이네 1", "돈삼이네 2" ,3,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member, company, "어셈블커피로스터즈", member.getName(), "뷰 좋은 카페 어셈블 커피 로스터즈", "삼랑진 근처 갔다가 추천 받고 간 곳. 커피도 맛있고 디저트도 맛있어요. 특히 뷰가 너무 예쁜데 넉동강 철교가 한 눈에 보여서 더 좋았어요~!^^ 딸기 계절이라 기대하고 딸기 제품을 시켰는데 냉동딸기 아닌 생딸기여서 식감도 당도도 너무 완벽했어요. 빵도 부드럽고 크림도 느끼하지 않고 너무 맛있는데 커피도 고소하니 너무 좋아요", "어셈블커피 로스터즈", "어셈블커피 로스터즈 2",3,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member3, company, "안동신세계찜닭", member3.getName(), "여러 명이서 갔는데 다들 너무 좋아했어요", "나름 7명이어서 예약하고 사장님께 전화드리고 갔는데 친구들이 늦게 와서 기다리고 있는데 바로 자리 만들어주시고 사람이 많았는데도 계속 여쭤봐도 친절히 대답해 주셨어요. 그리고 7명이서 대자로 안 맵게 부탁드린다고(매운 걸 못 먹어서...) 했는데 아이도 맛있게 잘 먹었어요! 그리고 저희 제일 큰 걸로 하나만 시켰는데 두 개로 주시길래 저희 큰 걸로 하나 시켰다고 하니까 사장님이 테이블이 2개라서 나눠서 주셨어요. 배려심에 너무 감동.. 양도 진짜많고 맛도 있어요. 또 방문할 예정이에요!", "안동 신세계 찜닭 1", "안동 신세계 찜닭 2",5,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member, company, "카페라파르",member2.getName(),"장자도를 간다면 꼭 한 번 들러보세요", "섬 안에 있는 카페 중에 약간 인스타 갬성 카페에요. 어디론가 떠나고 싶을 때 바람쐬기 좋은 곳이에요. 규모가 작아서 옹기종기 모여있는 느낌이지만 오선뷰라서 물멍을 할 수도 있고 그 덕에 힐링하기 좋은 느낌이에요. 한라봉 음료가 맛있어서 주문했는데 괜찮았고 디저트도 맛있어요. 장자도 가신다면 추천드려요.", "카페라파르", "카페라파르 2",8,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member2, company, "플러스아이미코병원",member2.getName(),"너무 친절하셔서 좋아요", "내과 진료 보러 갔는데 직원들도 친절하고 원장님도 자세한 설명에 이해가 잘 되었어요. 전체적인 분위기가 화기애애해서 더 좋았어요. 한동안 낫질 않아서 불편했는데 병원 바꾸고 여기서 처방받은 약을 먹으니 금방 좋아졌어요. 이전에 아이가 아파서 내원해서 진료 받을 때도 꼼곰하게 봐주시고 수액을 놓아주셔서 빠르게 호전되었었는데 앞으로 이 병원만 다녀야겠어요. ", "플러스아이미코병원 2", "플러스아이미코병원",2,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member, company, "NC 백화점 순천점", member.getName(), "쇼핑하기 좋아요", "가성비 좋은 예쁜 옷 구경하기도 좋고 주차도 편하고 브랜드도 다양해서 쇼핑하기 너무좋아요. 이번에는 얼마 전에 오픈만 젝시믹스를 다녀왔어요. 새로 오픈해서 오픈행사를 하고 있더라고요. 덕분에 저렴하게 득템할 수 있었어요. nc 순천점은 안에 밥집도 다양해서 쇼핑 후 허기지면 바로 밥을 먹으러 갈 수 있다는 것도 장점 아닌 장점이에요. 유아휴게실도 있어서 아이들 기저귀 갈기도 편하고 전자레인지도 있어서 아이들 밥 먹이기도 좋아요.", "NC 백화점 순천점", "NC 백화점 순천점 2",0,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member3, company, "핏앤피트니스", member3.getName(), "트레이너 분들이 너무 친절해요", "어깨가 많이 굽어있어서 운동하기 시작했는데 운동하면서 펴지는게 느껴지는게 너무 신기해요. 운동 시작할 때 헬스는 너무 지루하다는 생각이 있어서 엄청 고민했는데 고민을 왜 했나 싶을 정도로 꼼꼼하게 잘 봐주시고 친절하게 설명해주셔서 혼자 운동할 때도 잘할 수 있을 것 같아서 너무좋아요. 식단도 잘 짜주셔서 식단 지키는 것에 어려움도 없어요.", "핏앤피트니스", "핏앤피트니스 2",2,LocalDate.now().minusWeeks(2)));
        reviewPostRepository.save(new ReviewPost(member2, company, "테라로사 커피공장 강릉본점",member2.getName(),"커피를 좋아한다면 한 번쯤 가기 좋은 곳", "늘 교통편이 불편해서 가기 힘든 곳이어서 뚜벅이로 가려니 버스시간을 맞추긴 너무 힘들고 차를 가져가자니 장거리 운전이 부담되어서 몇 년간 언젠가는 가야지 마음만 먹고 있었는데 이번에 친구랑 강릉에 가면서 이번에는 버스를 타고 가지 말고 차를 끌고 가서 뚜벅이로 가기 힘든 곳을 가보자라고 마음 먹고 갔는데 고민을 많이하고 망설인게 무색할 만큼 카페는 너무 예뻤고 날도 너무 좋았어요. 그리고 옆에 레스토랑도 있는데 생각보다 너무 맛있었고 후식으로 커피를 마셨는데 커피 향도 너무 좋고 잘 내려주셔서 한 잔만 마시는게 아쉬울 정도였어서 다음에도 또 오고 싶다는 생각이 들더라고요. 그래서 조만간 또 갈 생각 입니다.", "테라로사", "테라로사 2",10,LocalDate.now().minusWeeks(2)));
    }
}
